# Classic_model data starts from.......Execute(IF WANT) FROM LINE 900 TILL END;(TO SELECT DATABASES)
--------------------------------------------------
USE CLASSICMODELS;
-- Day:3 -->
...
min(amount) over(partition by year(paymentDate)) 
as 'Min Amount' from payments;
...
from orders group by Quarter,year order by year;
...
-- Day:7 -->
	...
	# Q1:
select e.employeeNumber, concat(e.firstName,' ',e.lastName) as Sales_Person, 
count(c.salesRepEmployeeNumber) as Unique_Customers 
...
order by Unique_Customers desc;
	...
	# Q2:
select C.customerNumber, C.customerName, P.productCode, P.productName, OD.quantityOrdered as Ordered_Qty, 
P.quantityInStock as Total_Inventory ,(P.quantityInStock - OD.quantityOrdered)as Left_Qty
from Customers C 
inner join orders O on C.customerNumber = O.customerNumber 
inner join orderdetails OD on O.orderNumber = OD.orderNumber 
inner join products P on OD.productCode = P.productCode group by c.customerNumber,p.productCode order by c.customerNumber;

	# Q3:
create table Laptop(Laptop_Name varchar(30));
insert into Laptop values('Dell'),('Hp'),('Lenevo');

create table Colours(Colour_Name varchar(30));
insert into Colours values('White'),('Silver'),('Black');

select Laptop_Name, Colour_Name from Laptop inner join Colours order by Laptop_Name;
select count(*) as No_of_Rows from Laptop cross join Colours;
...
select mngr.FullName as Manager_name, emp.FullName as Emplyoyee_Name 
from Project mngr inner join Project emp 
...
Country varchar(100));
-- (I)
alter table facility modify column Facility_ID integer primary key auto_increment;
-- (II)
...
update UNIVERSITY set Name = concat(left(trim(name),instr(trim(name),' ')-1),' ',right(trim(name),10));
...
-- Day:10 -->
	...
	# Q1:
Create view PRODUCTS_STATUS as select year(orderDate) as YEAR,
Concat(count(quantityOrdered),' ','(',round((sum(quantityOrdered)/sum(sum(quantityOrdered))over())*100), '%',')') as VALUE
from orders join orderdetails using(orderNumber) group by YEAR order by quantityOrdered Desc;
...
-- Day:11 -->
	...
	# Q1:
DELIMITER 
CREATE PROCEDURE `GetCustomerLevel`(x integer)
BEGIN
	Declare l_v decimal(10,2);
	select creditLimit into l_v from customers where customerNumber = x;
	if
		l_v >= 100000 then 
		select concat('Customer ', x, ' is Platinum.') as CustomerStatus;
	elseif 
		l_v >= 25000 then
		select concat('Customer ', x, ' is Gold.')as CustomerStatus;
	elseif l_v < 25000 then
		select concat('Customer ', x, ' is Silver.')as CustomerStatus;
	else
		select 'Please enter valid CustomerNumber' as Warning;
	end if;
END 
DELIMITER ;
...
call GetCustomerLevel(103);
	...
	# Q2:
DELIMITER $$
CREATE PROCEDURE `Get_Country_Payments`(x int,y varchar(30) )
BEGIN

	select year(paymentDate) as year, country,
		concat(round(sum(amount)/1000),'K') as Total_Amount 
    from 
		payments join customers 
    using (customerNumber) 
    where 
		year(paymentDate)=x and country =y 
...
-- Day:12 -->
	...
	# Q1:
select year(orderDate) as Year,date_format(orderDate,'%M')as Month, count(customerNumber) as Total_Orders, 
/*lag(count(customerNumber),1) over (order by year(orderdate), month(orderdate)) as lag_time,*/ 
concat(round(((count(customerNumber)-lag(count(customerNumber),1) over (order by year(orderdate), month(orderdate)))/
(lag(count(customerNumber),1) over (order by year(orderdate), month(orderdate)))*100)),'%') as '%YoY Change'
from orders
...
BEGIN
	...
	DECLARE Y INT;
	DECLARE Z INT;
	SELECT TIMESTAMPDIFF(YEAR,DOB,NOW()) INTO X;
	SELECT TIMESTAMPDIFF(MONTH,DOB,NOW())%12 INTO Y;
	SELECT CONCAT(X,' Years ', Y, ' Monts') into Z;
	RETURN Z;
END //
DELIMITER ;

select *, age(DOB) as Age from emp_udf;
---------------------------------------------------
-- Day:13 -->
...
;
	...
	# Q3:
WITH CTE AS
(SELECT ORDERNUMBER, QUANTITYORDERED, DENSE_RANK() OVER (PARTITION BY ORDERNUMBER ORDER BY QUANTITYORDERED DESC)AS RANKS FROM 
ORDERDETAILS) 
SELECT ORDERNUMBER, QUANTITYORDERED FROM CTE WHERE RANKS=2;

	# Q4:
SELECT * FROM ORDERDETAILS;
SELECT MAX(TOTAL),MIN(TOTAL) FROM (SELECT ORDERNUMBER, COUNT(PRODUCTCODE) TOTAL 
FROM ORDERDETAILS GROUP BY ORDERNUMBER)PRODUCT_COUNT;
...
DELIMITER //
CREATE PROCEDURE `Update_Emp_EH`(x int, y varchar(30),z varchar(60))
BEGIN
	Declare continue handler for sqlexception
	BEGIN
		Select 'Error occurred'as Warning;
	END;
	insert into Emp_EH values(x,y,z);
	select 'Values Successfully inserted into table Emp_EH' as Message;
	select * from Emp_EH;
END

DELIMITER ;

call Update_Emp_EH(1,'Kalyan.KT','KtKalyan@gmail.com');
call Update_Emp_EH(2,'Jagadeesh.JD','Jagadeesh.L@gmail.com');
select * from Emp_EH;
-- Day:15 -->
